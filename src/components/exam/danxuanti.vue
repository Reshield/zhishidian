  ...
  data (){
    ...
    methods:{
        ...
        changecolor(index){
            ...
            let ifRight = this.items[index].right
            let answerIndex = this.answer.indexOf(option);//返回options在this.answer首次出现的位置，没有则返回 -1
            if(answerIndex >= 0){
            ...
            let oneAnswer = {}
            oneAnswer.option = option
            oneAnswer.ifRight = ifRight
            oneAnswer.page = this.page
            this.setAnswer(oneAnswer)
        },
